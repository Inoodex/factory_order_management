...
@section('title', 'Customers / Buyers')

@section('content')
    <div class="flex flex-wrap items-center justify-between gap-4">
        <h2 class="text-xl font-semibold uppercase">Customers & Brands</h2>
        <div class="flex w-full flex-wrap items-center justify-end gap-4 sm:w-auto">
            <a href="{{ route('admin.customers.create') }}" class="btn btn-primary gap-2">
            ...
            <form action="{{ route('admin.customers.index') }}" method="GET"
                ...
                <div class="relative w-full md:w-80">
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Search name, brand, season..." class="form-input ltr:pr-11 rtl:pl-11" />
                    <button type="submit"
                ...
                <div class="flex gap-2">
                    <select name="session" class="form-select min-w-[180px]">
                        <option value="">All Seasons</option>
                        @foreach ($sessions as $s)
                            <option value="{{ $s }}" {{ request('session') == $s ? 'selected' : '' }}>{{ $s }}</option>
                        @endforeach
                    </select>
                    <button type="submit" class="btn btn-primary">Filter</button>
                    <a href="{{ route('admin.customers.index') }}" class="btn btn-outline-danger">Reset</a>
                </div>
            </form>
        </div>
                ...
                <tbody>
                    @forelse ($customers as $customer)
                        <tr>
                            ...
                            <td class="font-semibold text-primary">{{ $customer->name }}</td>
                            <td>
                                @if($customer->brand)
                                    <span class="badge bg-info/10 text-info">{{ $customer->brand }}</span>
                                @else
                            ...
                            <td>
                                @if($customer->session)
                                    <span class="badge bg-secondary/10 text-secondary">{{ $customer->session }}</span>
                                @else
                                    <span class="text-gray-400">—</span>
                                @endif
                            </td>
                            ...
                            <td>
                                <a href="{{ route('admin.customer-orders.index', ['customer_id' => $customer->id]) }}" class="badge bg-primary text-white hover:opacity-80">
                                    {{ $customer->customer_orders_count }} Orders
                                </a>
                            </td>
                            <td class="text-center">
                                <div class="relative inline-block text-left" x-data="tableDropdown" @click.outside="close">
                                    ...
                                    <div x-show="open" x-cloak x-ref="menu"
                                        ...
                                        <a href="{{ route('admin.customer-orders.index', ['customer_id' => $customer->id]) }}"
                                            class="flex items-center gap-2 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-100 hover:text-primary dark:text-gray-200 dark:hover:bg-[#121e32] dark:hover:text-primary rounded transition">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-6 text-gray-500">No customers found. Click "Add Customer" to create one.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
